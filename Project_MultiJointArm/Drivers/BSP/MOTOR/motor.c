 ...
 */
 ...
 uint8_t Motor_Init(uint8_t mode, uint16_t acceleration, uint16_t deceleration, int speed, uint16_t communication_cycle_period)
 {
	  ...
	  uint8_t CommandData[8];
		...
		CommandData[6]=0x00; 
		CommandData[7]=0x00;	
		res = can_send_msg(0x700+ID_ElbowBendMotor, CommandData, 8);
		res = can_receive_msg(0x700+ID_ElbowBendMotor, CommandData);  // heartbeat detection
...
//		res = can_receive_msg(0x180+ID_ElbowBendMotor, CommandData);  // sync
		...
		CommandData[6]=0x00; 
		CommandData[7]=0x00;	
		res = can_send_msg(0x200+ID_ElbowBendMotor, CommandData, 8);
		printf("The result: %d\n", res);
...
uint8_t SetVelocity(int speed)
{
		...
		res = can_send_msg(0x200+ID_ElbowBendMotor, CommandData, 6);
...
uint32_t Get_CurrentPosition(void)
{
		...
		res = can_send_msg(0x600+ID_ElbowBendMotor, CommandData, 8);	
		delay_ms(10);
		res = can_receive_msg(0x0580+ID_ElbowBendMotor, CommandData);
