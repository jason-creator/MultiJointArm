...
/**
 * @brief       CAN 发送一组数据
 *   @note      发送格式固定为: 标准ID, 数据帧，CAN 标准帧信息为11个字节(3 + 8)
 * @param       id      : 标准ID(11位) "帧ID，000-7FF"
...
uint8_t can_send_msg(uint32_t id, uint8_t *msg, uint8_t len)
{
  ...
  g_canx_txheader.IDE = CAN_ID_STD;   /* 使用标准标识符 CAN_ID_STD  */
...
uint8_t can_receive_msg(uint32_t id, uint8_t *buf)
{
  ...
  {
		printf("id:%d, IDE:%d, RTR:%d\n", g_canx_rxheader.StdId, g_canx_rxheader.IDE, g_canx_rxheader.RTR);
		printf("Error\n");
    return 0;    
  }
